...
export async function getEquipmentbyName(name) {
  const [equipment] = await pool.query('SELECT * FROM equipment WHERE eq_name LIKE ?', [
    `%${name}%`,
  ]);
  console.log(equipment);
  return equipment;
...
export async function getCustomerbyFirstName(FirstName) {
  ...
  const [customers] = await pool.query('SELECT * FROM customer WHERE cus_fname LIKE ?', [
    `%${FirstName}%`,
  ]);
...
export async function getCustomerbyLastName(LastName) {
  const [customers] = await pool.query('SELECT * FROM customer WHERE cus_lname LIKE ?', [
    `%${LastName}%`,
  ]);
  console.log(customers);
  return customers;
...
export async function getCustomerbyAddress1(SAddress1) {
  const [customers] = await pool.query('SELECT * FROM customer WHERE cus_address1 LIKE ?', [
    `%${SAddress1}%`,
  ]);
  console.log(customers);
  return customers;
...
export async function getCustomerbyAddress2(SAddress2) {
  const [customers] = await pool.query('SELECT * FROM customer WHERE cus_address2 LIKE ?', [
    `%${SAddress2}%`,
  ]);
  console.log(customers);
  return customers;
...
export async function getInvoiceDetails(invoiceId) {
  const [invoice] = await pool.query(`SELECT * FROM customerInvoice WHERE cusinv_invid=?`, [
    invoiceId,
  ]);
...
export async function updateInvoiceDetails(InvoiceCompleteDetail) {
  ...
  try {
    await pool.query('INSERT INTO customerInvoice (cusinv_cusid, cusinv_invid) VALUES (?, ?)', [
